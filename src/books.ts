const BOOKS = [
  ...
  {
    isbn: "12234534",
    title: "Software Engineering in Rethinking Productivity ",
    subtitle: "",
    author: "Caitlin Sadowski, Thomas Zimmermann",
...
const authors = Object.keys(authorsTmp).map((author, i) => ({ id: String(i), name: author }));
const authorDict = Object.fromEntries(authors.map((author) => [author.name, author.id]));

const books = BOOKS.map((book, i) => {
  const authorId = authorDict[book.author];
  const d = { ...book, authorId };
  delete (d as any).author;
